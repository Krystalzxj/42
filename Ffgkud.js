...
if (!v4.primaryAddress && !v6.primaryAddress) {
  ...
  $httpClient.get('http://ip-api.com/json', function (error, response, data) {
    ...
    $done({
      ...
      content:
        `【𝑰𝑷 地址】\n` +
        (v4.primaryAddress ? `𝒗𝟒 : ${v4.primaryAddress} \n` : '') +
        (v6.primaryAddress ? `𝒗𝟔 : ${v6.primaryAddress}\n` : '') +
        (v4.primaryRouter && wifi.ssid ? `𝑹𝒐𝒖𝒕𝒆𝒓 𝒗𝟒 : ${v4.primaryRouter}\n` : '') +
        (v6.primaryRouter && wifi.ssid ? `𝑹𝒐𝒖𝒕𝒆𝒓 𝑰𝑷𝒗𝟔 : ${v6.primaryRouter}\n` : '') +
        `【节点 𝑰𝑷】: ${info.query}\n` +
        `【节点 𝑰𝑺𝑷】: ${info.isp}\n` +
