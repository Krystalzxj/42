...
;(async () => {
  ...
  await check_youtube_premium(), Promise.all([check_netflix()])
    .then((result) => {
  ...
  await inner_check()
    .then((code) => {
      if (code === 'Not Available') {
        ...
        youtube_check_result += '油管: ➺ ' + flags.get(code.toUpperCase() | + code.toUpperCase())
      }
    })
    .catch((error) => {
...
async function check_netflix() {
  ...
  await inner_check(81215567)
    .then((code) => {
      ...
      netflix_check_result += '奈飞: ➺ ' + flags.get(code.toUpperCase() | + code.toUpperCase())
